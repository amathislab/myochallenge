...
PATH_TO_NORMALIZED_ENV = "output/training/2022-10-25/09-49-10_4.5_5.5_random_small_noise_everywhere/training_env.pkl"  # "trained_models/normalized_env_original"

# Path to pretrained network (if not first task)
PATH_TO_PRETRAINED_NET = "output/training/2022-10-25/09-49-10_4.5_5.5_random_small_noise_everywhere/best_model.zip"  # "trained_models/best_model.zip"

# Tensorboard log (will save best model during evaluation)
now = datetime.now().strftime("%Y-%m-%d/%H-%M-%S") + "_noise_everywhere_s2"
...
config = {
    ...
    "limit_init_angle": np.pi / 6.0,
    "goal_time_period": [4.5, 5.5],  # phase 2: (4, 6)
    "goal_xrange": (0.022, 0.028),  # phase 2: (0.020, 0.030)
    "goal_yrange": (0.024, 0.030),  # phase 2: (0.022, 0.032)
...
if __name__ == "__main__":
    ...
    score_callback = EvaluateLSTM(
        eval_freq=10000, eval_env=env_score, name="eval/score", num_episodes=10
    )
    effort_callback = EvaluateLSTM(
        eval_freq=10000, eval_env=env_effort, name="eval/effort", num_episodes=10
    )
