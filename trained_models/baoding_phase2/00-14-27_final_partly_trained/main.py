...
PATH_TO_NORMALIZED_ENV = "output/training/2022-10-29/00-27-31_period_7_5_angle_pi_size_mass/training_env.pkl"  # "trained_models/normalized_env_original"

# Path to pretrained network (if not first task)
PATH_TO_PRETRAINED_NET = "output/training/2022-10-29/00-27-31_period_7_5_angle_pi_size_mass/best_model.zip"  # "trained_models/best_model.zip"

# Tensorboard log (will save best model during evaluation)
now = datetime.now().strftime("%Y-%m-%d/%H-%M-%S") + "_period_4_6_angle_pi_size_mass_friction"
...
config = {
    ...
    "goal_time_period": [6, 4],  # phase 2: (4, 6)
...
if __name__ == "__main__":
    ...
    score_callback = EvaluateLSTM(
        eval_freq=20000, eval_env=env_score, name="eval/score", num_episodes=25
    )
    effort_callback = EvaluateLSTM(
        eval_freq=20000, eval_env=env_effort, name="eval/effort", num_episodes=25
    )
    ...
    if FIRST_TASK:
        ...
        custom_objects = {  # need to define this since my python version is newer
            "lr_schedule": lambda _: 2.55673e-05,
            "clip_range": 0.3,
