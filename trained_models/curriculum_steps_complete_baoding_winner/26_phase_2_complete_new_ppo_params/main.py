...
PATH_TO_NORMALIZED_ENV = "output/training/2022-10-30/22-19-15_7-8_full-physics_angle-06pi_openai-params/training_env.pkl"

# Path to pretrained network (if not first task)
PATH_TO_PRETRAINED_NET = "output/training/2022-10-30/22-19-15_7-8_full-physics_angle-06pi_openai-params/best_model.zip"

# Tensorboard log (will save best model during evaluation)
now = datetime.now().strftime("%Y-%m-%d/%H-%M-%S") + "_final-everything_higher-lr"
...
if __name__ == "__main__":
    ...
    # effort_callback = EvaluateLSTM(eval_freq = 40_000, eval_env = env_effort, name = 'eval/effort', num_episodes=50)
    ...
    if FIRST_TASK:
        ...
        custom_objects = {  # need to define this since my python version is newer
            "lr_schedule": lambda _: 2.5e-04,
            "learning_rate": lambda _: 2.5e-04,
            "clip_range": 0.2,
            "n_steps": 4096,
            "batch_size": 4096,
            "ent_coef": 0.00025,
        ...
        print(f"\n\nModel learning rate: {model.learning_rate(1)}\n")
