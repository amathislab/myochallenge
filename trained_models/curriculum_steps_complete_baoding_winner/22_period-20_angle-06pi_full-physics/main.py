...
PATH_TO_NORMALIZED_ENV = "trained_models/baoding_phase2/07-26-20_period_20_angle_0_8pi_score70on08pi-80on06pi/training_env.pkl"

# Path to pretrained network (if not first task)
PATH_TO_PRETRAINED_NET = "trained_models/baoding_phase2/07-26-20_period_20_angle_0_8pi_score70on08pi-80on06pi/best_model.zip"

# Tensorboard log (will save best model during evaluation)
now = (
    datetime.now().strftime("%Y-%m-%d/%H-%M-%S") + "_20_06pi_full-physics_score-on-same"
)
...
config = {
    ...
    # 'obj_size_range': (0.020, 0.022),    #(0.018, 0.024   # Object size range. Nominal 0.022
    # 'obj_mass_range': (0.040, 0.100),    #(0.030, 0.300)   # Object weight range. Nominal 43 gms
    # 'obj_friction_change': (0.01, 0.001, 0.00002), # (0.2, 0.001, 0.00002) nominal: 1.0, 0.005, 0.0001
    "task_choice": "random",
}
...
if __name__ == "__main__":
    ...
    score_callback = EvaluateLSTM(
        eval_freq=40_000, eval_env=env_score, name="eval/score", num_episodes=50
    )
    ...
    model.learn(
        ...
        callback=[eval_callback, score_callback],
