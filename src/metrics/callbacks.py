...
class MyoInfoCallbacks(DefaultCallbacks):
    ...
    def on_episode_step(
        ...
        **kwargs,
        ...
        if info is not None:
            ...
            self.create_or_update_keys(
                episode.user_data, reward_info, list(reward_info.keys())
            )
    ...
    def on_episode_end(
        ...
        **kwargs,
        ...
        episode.custom_metrics.update(
            {
                f"{key}": np.mean(episode.user_data[key])
                for key in episode.user_data.keys()
            }
        )
        episode.custom_metrics.update({"solved": episode.user_data["solved"][-1]})
